...
class Adaboost:
    ...
    def applyFeatures(self, features, iis):
        ...
        print(f"len(features)={len(features)}, len(iis)={len(iis)}")
        for j in range(len(features)):
            for i in range(len(iis)):
                featureVals[j, i] = features[j].computeFeature(iis[i])
            if j % 1000 == 0:
              print(f"applied {j} features")
        print("out of applying")
        return featureVals
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        total_pos, total_neg = 0, 0                   #total_pos: the sum of the weights of all the car samples 
        for w, label in zip(weights, labels):         #total_neg: the sum of the weights of all the non-car samples 
            if label == 1:
                total_pos += w
            else:
                total_neg += w
        classifiers = []
        total_features = featureVals.shape[0]         #the number of features
        for index, feature in enumerate(featureVals):   #iterate through every feature
            if len(classifiers) % 1000 == 0 and len(classifiers) != 0:
                print("Trained %d classifiers out of %d" % (len(classifiers), total_features))
            applied_feature = sorted(zip(weights, feature, labels), key=lambda x: x[1])   # sort the weights and labels according to the feature value that they correspond to
            pos_seen, neg_seen = 0, 0
            #pos_weights: the sum of the weights of car samples so far
            #neg_weights: the sum of the weights of non-car samples so far
            pos_weights, neg_weights = 0, 0   
            min_error, best_feature, best_threshold, best_polarity = float('inf'), None, None, None
            for w, f, label in applied_feature:
                #evaluate the error of each possible threshold in constant time (O(1)) 
                #and the error of all thresholds in linear time (O(n))
                error = min(neg_weights + total_pos - pos_weights, pos_weights + total_neg - neg_weights)
                if error < min_error:
                    min_error = error
                    best_feature = features[index]
                    best_threshold = f
                    #The polarity is determined by how many positive examples and negative examples lie left (less than) 
                    #and right of the threshold (greater than)
            ...
            classifiers.append(clf)       #train all of the weak classifiers and store them in an array
        bestClf, bestError = None, float('inf')
        for clf in classifiers:           #find the best weak classifier among all of the weak classifiers
            error = 0
            for ii, w, label in zip(iis, weights, labels):
                correctness = abs(clf.classify(ii) - label)
                error += w * correctness
            error = error / len(iis)
            if error < bestError:
